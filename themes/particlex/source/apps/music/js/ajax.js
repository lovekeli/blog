...
function ajaxPlayList(lid, id, callback) {
    ...
    console.log('加载歌单L：',musicList);
    $.ajax({
        type: mkPlayer.method, 
        url: mkPlayer.api, 
        data: "types=playlist&id=" + lid,
        dataType : "jsonp",
        jsonpCallback: "callback_playlist_" +  lid,
        cache: true,
        complete: function(XMLHttpRequest, textStatus) {
        ...
        success: function(jsonData){
            ...
            if(jsonData.playlist.coverImgUrl !== '') {
                tempList.cover = jsonData.playlist.coverImgUrl + "?param=150y150";
            } else {
...
// ajax加载歌词
// 参数：音乐ID，回调函数
function ajaxLyric(music, callback, down) {
    if(!down) {
        lyricTip('歌词加载中...');
    };
    ...
    $.ajax({
	    ...
	    var lyric = jsonData.lyric || '';   // 原文歌词
            var tlyric = jsonData.tlyric || ''; // 翻译歌词
                         
            callback(lyric, tlyric, music.lyric_id, music);    // 回调函数
            //if (jsonData.lyric) {
            ...
            if(jsonData.playlist.length === 0 || typeof(jsonData.playlist.length) === "undefined")
            {
                ...
                for (var i = 0; i < jsonData.playlist.length; i++)
                {
                    ...
                    tempList = {
                        ...
                        cover: jsonData.playlist[i].coverImgUrl  + "?param=150y150",   // 列表封面
                ...
                // 显示退出登录的提示条
                //sheetBar(); 用insertCustomDivAfteruser();代替
