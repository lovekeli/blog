...
$(function(){
    ...
    // 点击专辑显示专辑歌曲
    $("#sheet").on("click",".music_co,.music_ti", function() {
        var num = parseInt($(this).parent().data("no"));
        // 是用户列表，但是还没有加载数据
    ...
    //搜索当前专辑所有歌曲
    $(".music-list").on("click", ".music-album", function () {
        rem.loadPage = 1; // 已加载页数复位
        rem.wd = $(this).html(); // 搜索词
	rem.wd = rem.wd.replace(/<i[^>]*>.*<\/i>/g, '');
        if (rem.wd == '专辑') return;
        layer.confirm('搜索当前专辑[<span class="wdname wdalbumname">'+rem.wd+'</span>]所有歌曲？', {
            btn: ['好的', '取消'],shadeClose: true,title: '搜索专辑' //按钮
        }, function () {
            ajaxSearch(); // 加载搜索结果
        }, function () {
           return;
    ...
    //搜索当前歌手所有歌曲
    $(".music-list").on("click", ".auth-name", function () {
        rem.loadPage = 1; // 已加载页数复位
        rem.wd = $(this).html(); // 搜索词
	rem.wd = rem.wd.replace(/<i[^>]*>.*<\/i>/g, '');
        if (rem.wd == '歌手') return;
        layer.confirm('搜索当前歌手[<span class="wdname wdauthname">'+rem.wd +'</span>]所有歌曲？', {
	    btn: ['好的', '取消'],shadeClose: true,title: '搜索歌手' //按钮
        }, function () {
            ajaxSearch(); // 加载搜索结果
        }, function () {
            return;
    ...
    $("#sheet").on("click",".login-in", function() {
        layer.prompt(
        {
            title: '请输入您的网易云音乐UID(用户id)',
            // value: '',  // 默认
            btn: ['确定', '取消', '帮助'],
            btn3: function(index, layero){
                layer.open({
                    title: '如何获取您的网易云音乐UID？'
                    ,shade: 0.6 //遮罩透明度
                    ,anim: 0 //0-6的动画形式，-1不开启
                    ,content: 
                    '1、首先<a style="color:#fde510" href="https://music.163.com/" target="_blank">(https://music.163.com/)</a>打开网易云音乐官网<br>' +
                    '2、然后登录您的账号<br>' + 
                    '3、点击您的头像&nbsp;&nbsp;>&nbsp;&nbsp;我的主页<br>' + 
                    '4、此时<span style="color:#cdcd39">浏览器地址栏</span> <span style="color:#00e2ff">/user/home?id=</span><span style="color:white;background:#ff246f;border-radius:5px;paddding:1px 4px;">数字</span><br>红色标记内的数字，就是您的网易云音乐用户UID'
                });  
            }
        },
...
function blog(){
    window.open('/apps/blog/')
};
function wall(){
    window.open('https://wall.wojc.cn/');
};
function github(){
    window.open('https://github.com/mengkunsoft/MKOnlineMusicPlayer')
};
...
function musicInfo(list, index) {
    ...
    '<span class="info-btn info-down" onclick="thisDownload(this)" data-list="' + list + '" data-index="' + index + '">歌曲</span>' + 
    '<span style="margin-left: 10px" class="info-btn info-wai" onclick="thisShare(this)" data-list="' + list + '" data-index="' + index + '">外链</span>' +
    '<span style="margin-left: 10px" class="info-btn info-lyricdown" onclick="thisDownloadLyric(this)" data-list="' + list + '" data-index="' + index + '">歌词</span>';
...
function searchBox() {
    ...
    '        <input type="text" name="wd" id="search-wd" placeholder="" autofocus required>' + 
...
// 下载歌词
// 下载正在播放的这首歌词
function thisDownloadLyric(obj) {
    //ajaxLyric(musicList[$(obj).data("list")].item[$(obj).data("index")], downloadLyric, 1);
}
...
function openDownloadDialog(url, saveName) {
    ...
    if (window.navigator && window.navigator.msSaveOrOpenBlob) {
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                window.navigator.msSaveOrOpenBlob(blob, saveName || 'download'); // IE 专用方法
            }
        };
        xhr.send();
    } else {
        // 其他现代浏览器使用 fetch 和 blob
        fetch(url)
        .then(response => response.blob())
        .then(blob => {
            var aLink = document.createElement('a');
            aLink.href = window.URL.createObjectURL(blob); // 将 blob 转换为 URL
            aLink.download = saveName || 'download'; // 设置下载文件名

            // 创建点击事件并触发下载
            document.body.appendChild(aLink); // 将 aLink 添加到 DOM
            aLink.click(); // 触发点击下载
            document.body.removeChild(aLink); // 下载后移除 aLink
	...
	layer.msg('正在下载歌曲，请稍后……', { icon: 6 });
    layer.closeAll('loading')
};

// 下载歌词参数：包含歌曲信息的数组(第二个str为翻译歌词)
function downloadLyric(str,tstr, id, music) {
    if (id !== musicList[rem.playlist].item[rem.playid].lyric_id) {
        layer.msg('请播放此歌曲后再点击下载歌词');
        return;
    };
    layer.msg('正在下载歌词，请稍后……', { icon: 6 });
    exportRaw(music.name + ' - ' + music.artist + '.lrc', str + tstr);
    layer.closeAll('loading');
};
        ...
        { 
            $("#music-cover").load(function(){
                if(animate) {   // 渐变动画也已完成
                    ...
                    $("#blur-img").animate({opacity:"0.4"}, 1000); // 背景更换特效
                } else {
            ...
            // 渐变动画
            $("#blur-img").animate({opacity: "0.1"}, 800, function(){
                if(imgload) {   // 如果图片已经加载好了
                    $("#blur-img").backgroundBlur(img);    // 替换图像并淡出
                    $("#blur-img").animate({opacity:"0.7"}, 2000); // 背景更换特效
                } else {
...
/**
 * 
 * @param {*} index  歌单的下标 1 为当前播放列表 2 为历史记录
 * @returns 
 */
//function loadList(index) {
function loadList(index, state){
    if(musicList[index].isloading === true) {
        layer.msg('列表读取中...', {icon: 16,shade: 0.01,time: 500});
        return true;
    };
    
    rem.dislist = index;     // 记录当前显示的列表

    dataBox("list");    // 在主界面显示出播放列表
    
    // 调试信息输出
    if(mkPlayer.debug) {
        if(musicList[index].id) {
            console.log('加载播放列表 ' + index + ' - ' + musicList[index].name + '\n' +
    ...
    if(state == 'init' && playerReaddata('playing') && (typeof playerReaddata('playing')) == 'string' && JSON.parse(playerReaddata('playing')).name){
        g_sheetTitle = JSON.parse(playerReaddata('playing')).name;
    } else if( index != 1) {//  如果是当前播放列表
        g_sheetTitle = musicList[index].name;
    };

    addListhead(g_sheetTitle);
    musicList[index].item.length;

    if(musicList[index].item.length == 0) {
        addListbar("nodata");   // 列表中没有数据
    } else {
        
        // 逐项添加数据
        for(var i=0; i<musicList[index].item.length; i++) {
            var tmpMusic = musicList[index].item[i];
            
            addItem(i + 1, tmpMusic.name, tmpMusic.artist, tmpMusic.album);
            
            // 音乐链接均有有效期限制,重新显示列表时清空处理
            if(index == 1 || index == 2) tmpMusic.url = "";
        };
        
        // 列表加载完成后的处理
        if(index == 1 || index == 2) {    // 历史记录和正在播放列表允许清空
...
// 播放列表滚动到顶部
function listToTop() {
    $("#main-list").animate({scrollTop: 0}, 100);
    //不适用滚动条插件
    /*if(rem.isMobile) {
...
// 向列表中加入列表头
function addListhead(title) {
    var html = '<div class="sheetlistname">当前歌单 <i class="fa fa-caret-right "></i><span class="listname">'+ title +'</span>&nbsp;<span class="sheetplay"><i class="fa fa-play-circle"></i>播放全部</span></div><div class="list-item list-head">' +
    ...
    $(".btnplaying").addClass("active");
    $(".sheetplay").click(function(){
        listClick(0);  //播放全部从头开始
	    $(this).hide();
    });
...
function addItem(no, name, auth, album) {
    ...
    '    <span class="music-album"><i class="fa fa-search fa-fw"></i>' + album + '</span>' +
    '    <span class="auth-name"><i class="fa fa-search fa-fw"></i>' + auth + '</span>' +
...
function addSheet(no, name, cover) {
    ...
    '    <div class="music_co"><img class="sheet-cover" src="' +cover+ '"></div>' +
    '    <div class="music_ti"><p class="sheet-name">' +name+ '</p></div>' +
    '</div>'; 
...
// 歌单列表底部登陆条
// sheetBar函数合并在insertCustomDivAfteruser()函数里面
/*function sheetBar() {
    var barHtml;
    if(playerReaddata('uid')) {
        barHtml = '<span class="btn login-btn login-refresh"><i class="fa fa-refresh fa-fw"></i>刷新</span><span class="btn login-btn login-out"><i class="fa fa-sign-out fa-fw"></i>取消</span> 已同步 <span class="username"> ' + rem.uname + ' </span> 用户的歌单 ';
    } else {
        barHtml = '<span class="btn login-btn login-in"><i class="fa fa-user fa-fw"></i>同步</span>&nbsp;加载你的网易云歌单';
    };
...
function initList() {
    ...
    for(var i=1; i<musicList.length; i++) {
        ...
        if(i == 1) {    // 正在播放列表
            // 读取正在播放列表
            var tmp_item = null;
            if (playerReaddata('playing')){
                if (typeof playerReaddata('playing') == 'string') {
                    var tmp_item = JSON.parse(playerReaddata('playing')).item;
                } else {
    ...
    if(musicList[mkPlayer.defaultlist].isloading !== true)  loadList(mkPlayer.defaultlist, 'init');
    // 显示最后一项登陆条
    //sheetBar(); 由insertCustomDivAfteruser();代替
};
