...
function pause() {
    if(rem.paused === false) {  // 之前是播放状态
        ...
        if(rem.playlist === undefined) {
            ...
            // 正在播放 列表项已发生变更，进行保存
            if (g_sheetTitle) {
                playerSavedata('playing', JSON.stringify({name:g_sheetTitle,item: musicList[1].item }));
            } else {
                playerSavedata('playing', JSON.stringify(musicList[1]));
            };
...
function audioPlay() {
    ...
    $(".lyric-name").text("《 " + music.name + " 》");   // 全屏下歌词上方歌曲名称
...
function updateProgress(){
    ...
    // 播放时间 / 总时间
	$('.mkpgb-ztime').text(formatTime(rem.audio[0].currentTime));
	if (!rem.audio[0].duration){ return };
	$('.mkpgb-ttime').text(formatTime(rem.audio[0].duration)) ;
};
...
// 初始化 Audio
function initAudio() {
    rem.audio = $('<audio id="audios"></audio>').appendTo('.footer');
...
var initProgress = function(){  
	...
	//新 增加歌曲时间显示
    $('#music-progress').append('<div class="mkpgb-ztime">00:00</div><div class="mkpgb-ttime">00:00</div>');
...
mkpgb.prototype = {
    ...
    init : function(){  
        ...
        // 加载进度条html元素
        $(mk.bar).html('<div class="mkpgb-bar"></div><div class="mkpgb-cur mkpgb-color"></div><div class="mkpgb-dot"></div>');
        // 获取偏移量
