...
const live2d_settings = {
    // 基本设置
    'modelUrl': window.config.live2dmodels,                        // 存放模型的文件夹路径，末尾不需要斜杠 window.config.live2dmodels https://clouddisk-1256223694.cos.ap-shanghai.myqcloud.com/live2dmodel
    'tipsMessage': '/apps/pets/waifu-tips.json',           // 看板娘提示消息文件的路径，可以留空不加载
    // 模型设置
    ...
    //看板娘样式设置
    'live2dHeight': 260,                        // 看板娘高度，不需要单位
    'live2dWidth': 200,                         // 看板娘宽度，不需要单位
    'waifuMinWidth': '200px',                 // 页面小于宽度小于指定数值时隐藏看板娘，例如 'disable'(禁用)，推荐 '1040px'
    ...
    'screenshotCaptureName': '看板娘.png',   // 看板娘截图文件名，例如 'live2d.png'
}
// 模型列表
const live2d_models = [
    {
        name: 'paimon',                                     // 模型名称要与文件夹名相同
        message: '派梦闪亮登场',  // 切换时的提示信息
...
function initModel() {
    ...
    if (pet != null || pet != undefined) {
        ...
        loadModel(JSON.parse(pet));
    } else {
        loadModel(null);
    }
}
function loadModel(model) {
    document.getElementById('waifu').style.display = (model == null || model == undefined) ? 'none' : 'block';
    if (model == null || model == undefined) return;
    let modelVersion = model.version;
    console.log(`加载模型 ${model.name}`);
    changePosition(modelVersion);
    // 在配置中找到要加载模型的版本
    ...
    if (modelVersion === 2) {
        ...
        window.live2dv2.load(live2dId2, `${live2d_settings.modelUrl}/${model.key}/model.json`);
    } else if (window.live2dCurrentVersion === modelVersion) {
        window.live2dv4.change(`${live2d_settings.modelUrl}/${model.key}`, `${model.key}.model3.json`);
    } else {
        $$(`#${live2dId4}`).style.display = 'block';
        window.live2dv4.load(live2dId4, `${live2d_settings.modelUrl}/${model.key}`, `${model.key}.model3.json`);
    }
...
window.addEventListener('message', (e) => {
    if (e.origin === window.location.origin) {
        ...
        switch (event) {
            case 'pets':
                loadModel(data);
                window.localStorage.setItem('pet', JSON.stringify(data));
                ; break;
        }
    }
});
